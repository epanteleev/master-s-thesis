...
\begin{figure}[h]
	...
	\caption{FlatArray с sentinel array}\label{first-fa}
\end{figure}
...
\begin{table}[H]
	...
	\end{tabular}}
\end{table}
Как видно, в случае not-existing и fifty-fifty хеш-таблица, основанная на FlatArray имеет лучшую производительность, чем стандартная HashMap. Однако в случае only-existing все наоборот. Нам бы хотелось иметь лучшую скорость операции поиска в хеш-таблице для not-existing и fifty-fifty, поскольку в реальных приложениях они встречаются чаще, чем случай
not-existing. Потому хотелось бы ускорить именно эти два случая.  
